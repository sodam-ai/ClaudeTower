...
- [ ] **`claudetower accounts`가 계정별 세션(5h)·주간(7d) 사용률·리셋 시각을 기본 표시**(2026-07-04 재확인 — teamclaude `session (5h) quota`/`weekly (7d) quota` 용어 채택, 공식 API 값이라 추가 조회 불필요. `--history`(로컬 집계, ccusage 벤치마킹)는 Phase 3 부가 기능으로 격하) — **부분 완료(M48, 2026-08-20 확인)**: 사용률(%)은 `accounts list`가 표시하지만, "5h/7d" 시간창 용어 대신 실제 API 키 헤더 모양 그대로(토큰/요청수 %, `quota-cache-store.js` 주석에 이유 명시 — API 키 헤더엔 시간창 개념이 없어 다른 이름을 붙이면 능력 과대고지가 됨) 표시. 리셋 시각(`tokens_reset_at`/`requests_reset_at`)은 캐시엔 이미 저장돼 있으나 `accounts list` 출력에는 아직 안 보여줌 — 그래서 줄 전체는 미완료로 유지.

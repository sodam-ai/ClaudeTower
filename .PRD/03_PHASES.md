...
- [x] ~~README·가이드 문서 작성 완료~~ → **2026-07-06 확인**: 04_PROJECT_SPEC.md Must Have 9개 항목 대조 완료(①/② 분리, 위험고지, FAQ·트러블슈팅·법률 모두 존재). 대조 중 실제 결함 발견: 명령어 목록이 PATH 미등록 상태(05번 문서 이슈#3, 아직 코드로는 미해결)를 반영 못 해 새 터미널에서 `claudetower` 명령이 안 먹히는 걸 README/GUIDE 어디도 경고하지 않았음 → README.md·README.en.md·GUIDE.md·GUIDE.en.md 4개 파일 모두 수정. **미완료로 남는 것**: 워크플로우 그림(①/② 시각 다이어그램)도 텍스트 설명만 있고 실제 그림은 없음(Should Have 수준으로 판단, 후속 과제). ~~GUIDE.pdf·GUIDE.en.pdf는 이번 .md 수정을 반영하지 못한 채 그대로임~~ → **2026-07-27 정정**: 두 PDF 파일은 이후 CHECKPOINT.md M7에서 저장소에서 완전히 삭제됨(더 이상 존재하지 않음, "갱신 필요" 서술 자체가 무효).
...
- [x] `claudetower accounts enable` — 이용약관 리스크 고지 + 명시적 동의 절차 **(M36, 2026-08-19 완료 — 2026-08-20 체크박스 정정)**
- [ ] 계정 등록(OAuth, 하이브리드: `oauth`/`api_key` 계정 병행 등록), `claudetower accounts add --import`(2026-07-27 추가, teamclaude 벤치마킹 — 기존 Claude Code 로그인 재사용), OS 자격증명 저장소 연동, `claudetower accounts`/`remove`/`rename`/`purge` CRUD — **부분 완료(2026-08-20 확인)**: OS 자격증명 저장소 연동·CRUD(add/list/remove/rename/purge)는 전부 완료. OAuth 계정 등록·`--import`만 여전히 throw-스텁(`.PRD/07_OAUTH_FLOW_SPEC.md §5-2`가 구현 세션 몫으로 남겨둔 부분) — 그래서 줄 전체는 미완료로 유지.
- [x] **`claudetower accounts`가 계정별 사용률·리셋 시각을 기본 표시**(2026-07-04 재확인 — teamclaude `session (5h) quota`/`weekly (7d) quota` 용어 채택, 공식 API 값이라 추가 조회 불필요. `--history`(로컬 집계, ccusage 벤치마킹)는 Phase 3 부가 기능으로 격하) — **완료(M48+M54, 2026-08-20 확인·정정)**: 사용률(%)은 `accounts list`가 표시하고(M48), 리셋 예정 시각(`tokens_reset_at`/`requests_reset_at`)도 이제 함께 표시한다(M54) — 이전 기록("리셋 시각은 아직 안 보여줌")은 M54 완료 후 이 문서를 갱신하지 않아 생긴 오기, M57에서 바로잡음. 단, "session(5h)/weekly(7d)" 시간창 용어는 쓰지 않는다 — 2026-08-19 결정(M41)으로 자동전환 대상을 API 키 계정으로만 한정했고, API 키 헤더엔 애초에 시간창 개념이 없어(`quota-cache-store.js` 주석 참고) 다른 이름을 붙이면 능력 과대고지가 된다. 이 용어 차이는 결함이 아니라 의도된 범위 재정의다.
- [x] **계정별 마지막 사용 프로젝트 경로 표시** — CLI 실행 시점 `process.cwd()`를 RotationEvent.project_path로 기록, 추가 탐색 없이 즉시 표시(Display 모듈을 거치지 않아 모듈 격리 유지) **(M53, 2026-08-20 완료 — `applySwitch`가 레지스트리의 `last_project_path`/`last_used_at`도 함께 갱신, `accounts list`가 표시)**
- [x] **`claudetower config`** — 전환 임계값·전략(`best`/`next-available`)·포트를 대화형/플래그로 조정, 값 검증 포함(claude-swap 벤치마킹) **(M33, 2026-08-17 완료 — 2026-08-20 체크박스 정정, `accounts-config-command.js`/`switch-policy-config.js` 확인)**
- [ ] **`claudetower`(또는 `eval $(claudetower env) claude` 패턴) — Claude Code를 프록시 환경변수가 이미 설정된 상태로 실행하는 핵심 진입점**(재시작 개념 자체가 없는 구조, teamclaude `run` 벤치마킹) — **CHECKPOINT.md 최상단 "🛑 실거래 배선 승인 게이트" 참고, 사용자의 별도 명시 승인 전까지 의도적으로 미착수**
- [ ] 로컬 프록시(127.0.0.1 고정, 로컬 접근 토큰 검증 — CLI가 직접 통제하므로 Claude Code 헤더 지원 여부와 무관), 할당량 기반 자동 전환, 429 failover — 순수 로직·프록시 서버 자체는 완성·격리 테스트 완료(`request-forwarder.js`, `active-account-provider.js`), 위와 동일한 배선 게이트로 실거래 연결만 보류
- [x] `RotationEvent` 감사 로그 **(M23/M30 완료 — 2026-08-20 체크박스 정정)**
- [x] `ActiveAccountHandle` 파일을 Account 모듈이 기록 시작 **(M46 완료 — 2026-08-20 체크박스 정정)**
...
- [x] **선택적 마켓플레이스 래퍼** — `/claudetower:*` 슬래시 명령이 설치된 CLI를 그대로 호출하는 얇은 Claude Code 플러그인. **2026-09-01 라이브 성공까지 완결(M65→M73)**: 저장소 루트 `.claude-plugin/`(plugin.json+marketplace.json)·`commands/`(status·widgets·config 3개, 원래 `plugin/` 폴더 안에 있었으나 `/plugin marketplace add` 라이브 테스트 실패로 루트로 이동, M67) → M70에서 진짜 원인(`main` 미병합)까지 확정·해소해 실제 `/plugin marketplace add`·3개 명령 라이브 성공 확인 → M71(Windows CRLF CI 결함 수정) → M72(자동완성에서 혼동 유발하던 구식 개인 스킬 `/claudetower-widgets` 영구 제거) → M73(PATH 미등록·미반영 상태에서도 명령이 고정 설치 경로로 폴백하도록 보강, Windows PATH 등록 자체의 방송 버그도 수정) — 마켓플레이스 발견성을 원하는 사용자를 위한 보조 채널일 뿐, 핵심 로직은 CLI에 그대로 있음(새 로직 0줄). `setup`(대화형 Y/n)은 슬래시 명령이 안전하게 흉내 내기 어려워 의도적으로 1차 범위에서 제외 — 상세는 `CHECKPOINT.md` M65·M67·M70~M73 참고
...
> **2026-07-27 정정**: 아래 표가 2026-07-15 상단 보류 배너 추가 시 갱신되지 않은 채 "시작 전"으로
> 방치돼 있었음(문서 나머지 전체와 모순). 이 문서 위쪽에 이미 서술된 사실만 반영해 정정.

| Phase | 핵심 기능 | 상태 |
|-------|----------|------|
| Phase 1 (MVP) | Display 모듈 + 격리 구조 확정 | **완료(2026-07-18)** |
| Phase 2 | Account 모듈(동의 기반 활성화, 하이브리드 OAuth+API키) | **대부분 완료(M23~M62, 2026-08-19~2026-08-21) — 2026-09-01 정정**: 동의 절차(M36)·CRUD(M48)·config(M33)·quota 표시(M48/M54)·프로젝트 경로(M53)·수동 전환(M59)·RotationEvent(M23/M30)·ActiveAccountHandle(M46) 전부 구현·테스트 완료. OAuth 계정 등록·`--import`(throw-스텁)와 실거래 배선(자동전환·프록시 토큰 검증·`claudetower run` 진입점)만 🛑 실거래 배선 승인 게이트로 의도적 보류. (이전 "실동작 착수 전" 서술은 바로 위 84~93행 상세 체크리스트와 반대 내용이라 문서 내부 모순이었음 — 상세 체크리스트 쪽이 맞았음) |
| Phase 3 | 고도화 + 계정 표시 연동 위젯 | **부분 착수** — Powerline 구분자(2026-07-18, 109행)·Git 위젯(2026-08-03, 브랜치+변경사항)·수동 강제 전환(M59)·`active_account` 위젯(2026-08-31, M62)·마켓플레이스 래퍼(2026-09-01, M65~M73, 라이브 성공까지 완결) 구현, 나머지(TUI 대시보드·핫 리로드·영구 셸 별칭·Powerline 색상 테마)는 미착수. **PR 위젯은 영구 제외**(`git.js` 주석 참고, 재론하려면 사용자 명시 요청 필요) |

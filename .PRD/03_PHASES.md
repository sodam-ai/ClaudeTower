...
- [x] **선택적 마켓플레이스 래퍼** — `/claudetower:*` 슬래시 명령이 설치된 CLI를 그대로 호출하는 얇은 Claude Code 플러그인. **2026-09-01 1차 완료(M65), 실사용 테스트로 경로 결함 발견·수정(M67)**: 저장소 루트 `.claude-plugin/`(plugin.json+marketplace.json)·`commands/`(status·widgets·config 3개, 원래 `plugin/` 폴더 안에 있었으나 `/plugin marketplace add` 라이브 테스트 실패로 루트로 이동) — 마켓플레이스 발견성을 원하는 사용자를 위한 보조 채널일 뿐, 핵심 로직은 CLI에 그대로 있음(새 로직 0줄). `setup`(대화형 Y/n)은 슬래시 명령이 안전하게 흉내 내기 어려워 의도적으로 1차 범위에서 제외 — 상세는 `CHECKPOINT.md` M65·M67 참고

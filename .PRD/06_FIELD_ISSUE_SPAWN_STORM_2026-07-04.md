...
> **2026-09-01 정정(M72)**: 이 문서 안의 `/claudetower-widgets`(개인 스킬) 언급은 이제
> 존재하지 않는 기능이다 — 05번 문서 상단 정정 참고, 소스 코드에서 영구 제거했다
> (`CHECKPOINT.md` M72). 갱신 속도 조절 자체는 `claudetower config statusline-refresh`
> 또는 마켓플레이스 `/claudetower:config`로 여전히 가능하다.
...
| ~~FR-5~~ 대체됨 | **조절 실패 지점 규명** — 2026-07-06: `claudetower config statusline-refresh` 명령이 생기면서 "사용자가 settings.json을 손으로 못 고쳤던 이유(A/B/C 가설)" 자체가 무의미해짐. 이제는 손으로 안 고치고 전용 명령으로 안전하게 고치므로 §2.2 가설 규명은 더 이상 필요 없음 | P1 조사 |

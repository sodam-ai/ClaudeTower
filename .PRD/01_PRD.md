...
> **[2026-07-27 재개]** 위 법적 근거는 그대로 유효하나, 사용자가 GitHub `jung-wan-kim/teamclaude`
> 실측 분석을 근거로 재개를 요청했고, 우회 설계도 동일하게 걸린다는 조항까지 다시 고지한 뒤
> 위험을 인지·수용하고 하이브리드(OAuth+API키)로 진행하기로 최종 확정했다(`07_OAUTH_FLOW_SPEC.md
> §5`, `CHECKPOINT.md` 트랙3 참고). 이 문서의 Phase 2 관련 서술은 역사 기록이자 **재개 이후
> 계속 유효한 설계**다 — 다만 실제 구현은 다음 세션부터다.
...
- [x] Account 모듈 활성화 전에는 OAuth 관련 코드·자격증명 저장소 접근이 전혀 일어나지 않음 — **2026-09-01 정정(M36 완료 반영)**: `ModuleActivationState.enabled`가 false인 동안 Account 프로세스 자체가 기동되지 않도록 구현·테스트됨(`module-activation-state.js`). 이전 "구현 미착수" 표기는 M36(2026-08-19) 완료를 반영하지 못한 오기 — `03_PHASES.md` 상세 체크리스트와 대조해 발견·정정
- [x] `claudetower accounts enable` 동의 절차 없이는 계정 등록이 시작되지 않음 — **2026-09-01 정정(M36 완료 반영)**: `consent-text.js`+`accounts-enable-command.js`로 이용약관 리스크 고지+명시적 동의 절차 구현·테스트 완료. 이전 "구현 미착수" 표기는 위와 동일한 오기
- [ ] Windows(PowerShell만 있는 환경)에서 Display 모듈이 폴백 없이 정상 작동
- [ ] Account 모듈 활성화 후 자동 전환·429 failover가 실제 계정으로 검증됨 — **2026-09-01 정정**: 순수 로직·프록시 서버 자체는 완성·격리 테스트 완료(`request-forwarder.js`, `active-account-provider.js`), 다만 CHECKPOINT.md "🛑 실거래 배선 승인 게이트" — 사용자의 별도 명시 승인 전까지 실거래 연결은 의도적 보류(이전 "다음 세션" 표기는 게이트 성격을 반영 못 한 오기)
- [ ] 로컬 프록시 접근 토큰 없이 보낸 요청은 거부됨 — **2026-09-01 정정**: 위와 동일 사유(🛑 실거래 배선 승인 게이트)로 실거래 검증 보류, "다음 세션" 표기 정정
- [ ] **(신규)** `claudetower`(또는 등록된 셸 별칭)으로 Claude Code를 실행하면 별도 재시작 없이 즉시 프록시가 개입함 — **2026-09-01 정정**: `claudetower run` 진입점 자체가 🛑 실거래 배선 승인 게이트 대상이라 착수 자체가 불가능한 상태, "다음 세션" 표기 정정

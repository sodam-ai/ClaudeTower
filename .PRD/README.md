...
- [ ] 프로젝트 최종 이름 확정("ClaudeTower"은 가제)
- [ ] 라이선스·저작권자, 배포 채널
- [ ] **(최우선, 법무 검토 필요)** Account 모듈 포함으로 인해 이용약관 리스크가 플러그인 전체에 걸리는 문제 — README에서 "Account를 켜지 않으면 무관하다"를 명확히 구분 설명해야 함
- [ ] Display 모듈만 담은 경량판 별도 배포 여부
- [ ] 로컬 프록시 접근 토큰 구현 방식 실측 필요
- [ ] OS별 자격증명 저장소 라이브러리 선택, Windows 코드서명 여부
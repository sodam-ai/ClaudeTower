...
> **코드 자체는 삭제되지 않고 저장소에 그대로 남아있다**(2026-07-27 실측 정정 — 이전에 "삭제됐다"고
> 적혀 있었으나 사실 아님, `ls src/accounts/` 직접 확인). 다만 CLI 어디에서도 로드/import되지
...
> **2026-08-20 결정(범위 축소)**: 위 "자격증명 저장" 행의 Linux libsecret은 더 이상 목표가
> 아니다 — 사용자가 물리 macOS/Linux 환경이 없어 이 세션까지 라이브 검증이 불가능했던
> 상태(CHECKPOINT M40 잔여 위험)에서, "Linux는 Account 기능(계정 등록·자동전환) 자체를
> 지원하지 않는다"로 범위를 명시적으로 좁혔다. `src/accounts/credential-store/index.js`가
> Linux에서 명확한 오류로 즉시 거부하도록 구현됨(추측이 아니라 실제 코드). **Display
> 모듈은 이 결정과 무관하게 계속 Linux를 지원한다** — "Windows/Mac/Linux 동일 코드"
> 원칙은 Display 모듈에서는 그대로 유지되고, Account 모듈에서만 예외가 생긴 것이다.
> macOS는 사용자가 실제 기기를 확보하면 그때 별도로 포팅·검증할 예정(보류가 아니라
> "아직 검증 수단이 없어 순연"에 가까움).
...
- [x] ~~Display 모듈만 담은 경량판을 별도 배포할지 여부~~ → **N/A(2026-07-15, 2026-07-26 문서 정합화)**: `01_PRD.md` §7이 이미 "Account 모듈이 영구 보류되어 이제 ClaudeTower 자체가 이미 경량판"으로 답했으나 이 문서만 미해결로 남아있던 잔여 불일치 — 01_PRD.md와 동일하게 정정
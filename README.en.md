...
A statusline CLI for Claude Code. Written so that anyone can follow it from start to finish, even with little or no prior experience with computers or AI tools — this single document covers everything from installation to troubleshooting and legal information. (An experimental feature for registering API-key accounts and manually switching between them is included but disabled by default; login-account automation and fully automatic, hands-off usage-triggered switching still don't exist — see "② Account switching" below.)

> **Current status (important)**: "① Statusline" below is always safe and works right now. "② Account switching" is **already included in the currently released v0.5.3** — you can register, list, delete, and rename API-key accounts, and manually switch between registered accounts by running a command yourself (`accounts switch`), and it is **always disabled by default** — you must read the consent notice and explicitly opt in via `accounts enable`. Login (subscription) account automation still has no code at all, since it still conflicts with Anthropic's official Terms of Service, and there is still no fully automatic (hands-off) usage-triggered switching (see "② Account switching" below for details).
>
...
> `accounts` commands have already been included since v0.5.0, but are **disabled by default** — until you run `accounts enable` to consent, everything except `status`/`config` lookups is refused. This is a fully separate feature from the statusline; see "② Account switching" below for details. Commands actually included in the current release (v0.5.3): `accounts status` (check components) · `accounts config` (pre-set switch thresholds etc.) · `accounts enable` (opt in after reading consent) · `accounts add --api-key <label> <key>` (register an API-key account) · `accounts list` (view registered accounts) · `accounts remove <label>` (delete one account) · `accounts rename <old-label> <new-label>` (rename it) · `accounts switch <label>` (manually switch to one of your registered accounts, since v0.5.3) · `accounts disable` (turn back off) · `accounts diagnose-quota <label>` (check real usage) · `account-purge` (delete every registered account at once, `[y/N]` confirmation required and irreversible — a top-level command, not a subcommand of `accounts`, so run it as `claudetower account-purge`).
...
│   └── accounts/              # Account registration/switching code — included in the exe since v0.5.0, manual switch since v0.5.3 (disabled by default; see "② Account switching" above)
...
**Already included in the current release (v0.5.3)** (disabled by default — see "How to turn it on" below first, **Windows/macOS only**):
...
- `claudetower accounts remove <label>` — deletes a single account (`[y/N]` confirmation required, irreversible). To delete every registered account at once, use `account-purge` (a top-level command, not a subcommand of `accounts` — run it as `claudetower account-purge`; this also requires `[y/N]` confirmation)
...
- **`claudetower accounts switch <label>`** (since v0.5.3) — switches to one of your registered accounts when **a person runs the command themselves** (manual switching, not automatic detection). It applies immediately with no confirmation step needed (it's reversible — just `switch` again — so it's not risky the way deletion is), only works between API-key accounts (you cannot switch to a login/subscription account), and each switch is recorded with when it happened and which account it switched from/to. Right after switching, the statusline's "Active account" item updates immediately too (see the "① Example" above)
...
- **The program automatically switching accounts on its own (fully automatic, hands-off)** — even with multiple accounts registered, nothing automatically switches to the next account when usage runs out today; only running `accounts switch` yourself (immediately above) changes it. This item means only the automation piece is missing — manual switching itself is already in the current release
...
Officially released versions (current latest: v0.5.3). Click to expand.

<details>
<summary><strong>v0.5.3</strong> — Manual account switching + official marketplace plugin support (latest)</summary>

**New user-visible features**: `claudetower accounts switch <label>`, letting you manually switch to one of your registered API-key accounts, ships in an official release for the first time — switching makes an "Active account" item appear in the statusline (still stays completely invisible for the vast majority of users who never turn the account feature on). `accounts list` now also shows each account's last-used time and project folder, plus when its usage is due to reset. The Claude Code marketplace plugin (`/plugin marketplace add sodam-ai/ClaudeTower`) reached live verification and is now officially supported for the first time — running `/claudetower:widgets` with no arguments now shows a check-box selection menu right in the chat.

**Important reliability fixes**: Fixed the root cause of Windows PATH registration succeeding during `claudetower setup` but not actually taking effect in newly opened terminals (the OS was never told "the environment variables changed") — no reboot is needed anymore. Added a security check to the local proxy that rejects browser-originated requests (Origin/Referer header inspection). Applied file locking and atomic writes consistently across every file holding account data, so data stays intact and isn't lost even with multiple terminal windows open at once. Also fixed a race condition where registering the same label from two terminals at nearly the same time could create a duplicate account entry.

**Known limitations (same as v0.5.0)**: Login (subscription) account automation is still not built, due to the Terms-of-Service issue. There is still no fully automatic feature that detects usage and switches on its own — you still need to run `accounts switch` yourself. macOS still has no real-device verification (build and smoke tests are confirmed via CI only), and Linux still doesn't support the account feature at all.
</details>

<details>
<summary><strong>v0.5.0</strong> — Account registration & management officially included</summary>
...
- **Are there plans to build account switching?** Automatically cycling login (subscription) accounts has been found to conflict with Anthropic's Terms of Service, so it won't be built. That said, "registering" API-key accounts (since v0.5.0) — the path the Terms of Service carve out as an exception — and manually switching between registered accounts yourself (`accounts switch`, since v0.5.3) are already included in the official release, disabled by default. The program automatically detecting usage and switching fully on its own, with no human involved, is still not planned (see "② Account switching" above for details).

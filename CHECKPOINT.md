...
- [x] **CT-S2** **README의 갱신 주기 안내 보완** — **2026-09-01 적용 완료(M63)**
      ...
      **문제 3 — 근거 수치가 없다**
      ...
      - **2026-09-01 완료 확인**: 위 "넣을 내용" 4가지 전부 반영, 영어판도 실제로 대조해
        한국어판과 동일 구조임을 확인 후 동일 내용 반영(추정이 아니라 확인 후 작업). 상세는
        M63 참고.
...
## M63: 2026-09-01 — 문서 정합성 마감 (README.html/README.en.html 재생성 + CT-S2 갱신주기 안내 정정)

**착수 전 확인**: `git fetch` + `git rev-list --left-right --count origin/docs-and-fixes/2026-07-06...HEAD`
→ `0 2`(origin에만 있는 커밋 0개, HEAD에만 있는 커밋 2개 = M62의 두 로컬 커밋 그대로) — 동시
세션 없음, 새 커밋은 오직 이 세션이 만든 것뿐임을 확인 후 시작.

**배경**: M62가 "남은 위험"으로 스스로 남긴 두 가지 문서 부채를 마감했다 — ① README.html/
README.en.html이 README.md/README.en.md보다 뒤처진 상태(M61이 만든 pandoc 파이프라인 재사용
예고), ② 2026-08-30 `/doctor` 8회차(CT-S2)가 진단만 하고 미적용 상태로 남겨둔 README 갱신주기
안내 정정(done-when·넣을 내용까지 이미 문서에 명시돼 있었음). 둘 다 AI가 새로 만든 범위가
아니라 CHECKPOINT.md 자신이 이미 "다음에 할 일"로 못박아 둔 항목이라, 코드 변경 없이 낮은
리스크로 마감 가능하다고 판단해 이번 라운드로 선택했다(Powerline 색상 테마·Git/PR 위젯 같은
진짜 신규 기능보다 확실성이 높다는 근거로 사용자에게 먼저 제안 후 승인받고 진행).

**만든 것**:
- [x] `README.md:145`·`README.en.md:145`(명령어 설명) — 조건부("세션을 여러 개 띄워두는
  경우") 5초 권장을 **무조건 5초 권장**으로 상향, 실측 근거(1초 7.6%/3초 2.5%/5초 1.5%, CPU
  점유율) 명시, 제품 자체 권장범위(2~5초, `.PRD/05_FIELD_ISSUES_2026-07-04.md:197`) 인용.
- [x] `README.md:219`·`README.en.md:219`(워크플로우 1단계) — "정해둔 주기마다만 갱신된다"로
  오독되던 서술을 **이벤트 기반 갱신(즉시) + 유휴 상태 백업 타이머(N초)** 2단 구조로 정정.
  이 오독은 "짧을수록 반응이 빠르다"는 반대 방향 오해를 유발해 2026-07-04 스폰 폭주와 같은
  경로로 이어질 수 있다는 것이 CT-S2가 이미 밝혀둔 위험이었다.
- [x] `README.html`/`README.en.html` 재생성 — M61과 동일한 방식(`pandoc -f gfm -t html
  --wrap=none`으로 본문 변환 + 기존 CSS 스타일 셸(다크모드 포함)에 이어붙이기 + 자기참조
  링크 `./README(.en).md` → `./README(.en).html` 후처리)으로, 위 CT-S2 반영분과 더불어
  M62(activeaccount 위젯) 문구까지 함께 최신화했다 — `.html` 두 파일 모두 M62 시점부터
  이미 뒤처져 있었기 때문.

**작업 중 발견한 사고(정직하게 기록, 실제 파일 덮어쓰기 전에 자체적으로 잡음)**: HTML 재조립용
셸(head 750바이트 분량)을 처음 `/tmp/head_ko.html`에 추출했더니, 실제로는 **이 프로젝트와
무관한 다른 프로젝트(Wikimate)의 CSS·제목이 담긴 파일**이 들어 있었다 — 이 세션의 Bash 도구가
쓰는 `/tmp`가 세션 전용 격리 공간이 아니라 다른 작업에서도 재사용되는 공유 경로였던 것으로
보인다(원인 확정은 아니고, 재현된 증상만 확정: `head -n 47 README.html`을 실행했는데 결과
파일에 엉뚱한 프로젝트 내용이 있었음). **실제 README.html에는 아직 반영 전이었고**, 재조립
결과를 실제 파일에 쓰기 직전 `diff`로 대조하는 습관 덕분에 배포 전 발견했다 — 이 세션의
시스템 지침이 원래 요구하는 대로 세션 전용 스크래치패드 경로로 다시 작업해 해소했다(이후 모든
산출물을 스크래치패드 경로에서 만들고 `diff`로 재검증한 뒤에만 실제 파일에 복사). **교훈**:
`/tmp`를 신뢰하지 말고 항상 지정된 스크래치패드 경로를 쓸 것 — 이번엔 다른 프로젝트 문서가
잘못 섞여 들어갈 뻔한 것뿐이었지만, 원인이 다른 세션과의 경합이라면 데이터 유실 등 더 큰
사고로 이어질 수 있는 종류의 위험이었다.

**검증(전부 직접 실행, 자기선언 아님)**:
- 실제 파일 덮어쓰기 전 스크래치패드 산출물 기준: `**`(미변환 마크다운) 잔존 0건(한국어·영어
  둘 다), `<table>` 8개·`<details>` 15개 개수 기존과 완전 일치, `<h2 id="...">` 섹션 목록
  `diff`로 완전 일치(섹션 손실 없음).
- 핵심 문구 11개(`accounts switch`/`v0.5.0`/`활성 계정`/`recommended value is 5s`/
  `idle-time backup timer` 등)를 `.md` 원본과 등장 횟수로 대조 — 전부 일치.
- 실제 파일 복사 후 `git diff`로 의도한 4곳(명령어 설명·워크플로우 1단계, 한국어·영어 각
  2곳)과 M62가 이미 만들어뒀던 "활성 계정" 관련 두 문단(HTML만 뒤처져 있던 부분)만 변경됐음을
  `README.html`/`README.en.html` 전체 diff로 직접 확인 — 그 외 섹션은 문자 그대로 무변경.
- `npm run lint`: 클린(코드는 건드리지 않음, 재확인 목적). `git status`로 의도한 4개 파일
  (README.md/README.en.md/README.html/README.en.html) + 이 CHECKPOINT.md 외 변경 없음 확인.

**의도적으로 하지 않은 것**: `.PRD/` 문서는 이번 범위 밖(README는 사용자 대상, `.PRD/`는
개발자 설계 기록 — M61과 동일한 구분 원칙 유지). Powerline 색상 테마·Git/PR 위젯 등 실제
신규 기능은 이번 라운드에 포함하지 않았다(사용자에게 문서 정합성 마감을 먼저 제안해 승인받은
범위만 진행, 코드 스코프 확장 안 함).

**남은 위험**: 낮음. `.md`↔`.html` 드리프트는 이번에도 수동 pandoc 실행에 의존하므로(M61이
남긴 것과 동일한 자동화 부재), 다음에 README.md를 고치는 세션이 `.html` 재생성을 잊으면 같은
패턴이 다시 쌓인다 — `package.json` 스크립트로 자동화하는 것은 이번 범위 밖으로 판단했다.

- 상태: **완료** — `README.md`·`README.en.md`·`README.html`·`README.en.html`·`CHECKPOINT.md`
  변경, 코드 변경 없음. 로컬 커밋만(push는 사용자가 이후 결정).

---

## M64: 2026-09-01 — PRD 문서 정정 (`Git/PR 위젯` 표기가 이미 끝난 결정을 다시 열어보이게 만들던 문제 해소)

**착수 전 확인**: `git fetch` + `git rev-list --left-right --count origin/docs-and-fixes/2026-07-06...HEAD`
→ `0 3`(origin에만 있는 커밋 0개, HEAD에만 3개 = M62+M63 그대로) — 동시 세션 없음.

**배경**: M63 직후 "다음 작업" 후보를 찾던 중, Powerline 색상 테마는 사용자가 "이번엔 보류"를
선택했고, 이어서 Git/PR 위젯을 검토하다가 **같은 세션 안에서 이 항목을 세 번이나 다시
검토하는 헛수고**를 겪었다(색상 테마 다음 대안으로 1번, "timeout을 걸면 되지 않을까" 완화안
검토로 1번). 세 번째에 `src/display/widgets/git.js` 헤더 주석을 자세히 읽고서야, PR 제외가
이미 다른 세션이 "gh CLI 등 완전히 새로운 외부 의존성 필요, Simplicity First 판단, **사용자
사전고지 후** 결정"이라고 확정지어 둔 사안임을 발견했다 — 재검토할 문제가 아니라 이미 끝난
결정이었다.

**원인**: PRD 5곳(`01_PRD.md` 2곳, `03_PHASES.md` 2곳, `02_DATA_MODEL.md` 1곳)이 "Git/PR
위젯"을 하나로 묶어 표기하고 있었는데, `02_DATA_MODEL.md`만 "PR 상태는 제외"를 정확히
반영했고 나머지 4곳(그중 `03_PHASES.md`의 실제 상태 추적 절 2곳 포함)은 여전히 통째로
"미착수"라고 서술 중이었다 — **Git 위젯(브랜치+변경사항)이 2026-08-03에 이미 완료·배포돼
실사용 중인데도** 그 사실이 로드맵 어디에도 반영돼 있지 않았다. 문서가 실제보다 뒤처진 채로
방치되면서, "아직 안 한 걸로 보이는" PR 제외 결정을 매번 새 검토 대상으로 착각하게 만든
것이 이번 헛수고의 직접 원인이다.

**만든 것**:
- [x] `.PRD/01_PRD.md` 93행 — Git/PR 위젯 표에 정정 각주 추가(Git 완료일·검증 근거, PR
  영구 제외 근거와 출처를 명시).
- [x] `.PRD/03_PHASES.md` 115행 — 하나로 묶여 있던 체크박스를 둘로 분리: Git 위젯(`[x]`,
  완료·검증 근거·PR 제외 사유 전부 명시) / Powerline 색상 테마(`[ ]`, 오늘 사용자가 "보류"
  선택했다는 사실까지 기록).
- [x] `.PRD/03_PHASES.md` 136행 — Phase 로드맵 요약표의 "구현" 목록에 Git 위젯을 추가하고
  "미착수" 목록에서 제거, PR 위젯은 별도로 "영구 제외"라고 명시.
- **의도적으로 안 건드린 곳**: `.PRD/01_PRD.md` 132행("안 만드는 것" 목록의 Phase 1 시점
  제외 사유 — 여전히 정확한 역사적 기록이라 정정 불필요), `.PRD/02_DATA_MODEL.md` 69행(이미
  "PR 상태는 제외"로 정확히 서술돼 있어 정정 불필요), `.PRD/04_PROJECT_SPEC.md` 211·236행
  (PR 위젯을 "혹시 나중에 재론되면" 지켜야 할 기술 요구사항으로 조건부 서술한 것이라, PR이
  영구 제외됐다고 해서 이 요구사항 자체가 틀린 건 아님 — 손대지 않음).

**검증**: 5곳 전부 grep으로 재대조해 "Git 위젯=완료", "PR=영구 제외(재론하려면 사용자 명시
요청 필요)"라는 동일한 사실을 서로 다른 말로 반복해서 서술하되 모순 없이 일치하는지 확인.
코드 변경 없음(문서만) — `npm run lint` 재확인(클린, 재확인 목적).

**의도적으로 하지 않은 것**: Powerline 색상 테마·TUI 대시보드·핫 리로드·`accounts --history`
자체를 착수하지 않음(전부 사용자 결정 또는 원칙 충돌이 필요한 상태 그대로) — 이번 라운드는
순수하게 "이미 끝난 결정이 문서 때문에 다시 열려 보이는" 문제 하나만 해소했다.

**남은 위험**: 낮음. 앞으로 Git 위젯에 정말 새 기능(예: 원격 저장소 ahead/behind 표시 등)이
추가되면 이번에 "완료"로 닫아둔 체크박스를 다시 열어야 한다 — 그 시점에 이 M64 기록을 먼저
참고할 것.
...
## M68: 2026-09-01 — `.PRD/04_PROJECT_SPEC.md`에 M67 미반영 잔여 drift 2곳 정정 (문서 전용, 코드 무변경)

**왜**: PRD 전수 재정독 요청에 따라 CHECKPOINT.md M39~M67 전체와 PRD 8개 문서를 서브에이전트로
전수 재조사한 뒤, 메인 세션이 직접 원문(`grep -n "plugin/" .PRD/04_PROJECT_SPEC.md`)을
대조해 확인했다. M67이 `.PRD/04_PROJECT_SPEC.md §261`(3순위 채널 설명 문단)은 정확히
정정했지만, **같은 파일 안의 다른 두 지점은 옛 `plugin/` 경로를 그대로 남겨뒀다**는 걸
발견했다 — M32·M64·M67 자신이 이미 겪었던 "PRD 일부만 정정하고 나머지는 놓치는" 패턴이
M67에서도 반복된 것.

**발견한 잔여 drift**:
1. 프로젝트 폴더 구조 다이어그램(구 73~76행) — `plugin/└── .claude-plugin/`으로 그려져
   있어 마켓플레이스 파일이 여전히 `plugin/` 안에 있는 것처럼 보였음.
2. 배포 현황 표(구 276행) — "1차 완료(2026-09-01, M65) | `plugin/` 폴더 —..."로 M65만
   인용하고 M67 이동·재검증 대기 상태를 반영하지 못했음.

**왜 이 작업을 선택했나(다른 후보 대비)**: 서브에이전트 조사가 제시한 후보 중
①마켓플레이스 라이브 재검증(사용자의 push+재시도 필요, AI가 완결 불가)
②`diagnose-quota` 실측(실비용+사용자 계정 필요, AI가 자체 판단으로 실행 불가)
③TUI 대시보드(전제조건인 실사용 데이터가 여전히 희박, 범위도 큼)
④`add-api-key` 락 재구조화(임팩트 낮음, 과설계 위험) — 넷 다 이번 세션에서 안전하게
"진행→완료"까지 못 끝내거나 범위가 과도했다. 이 drift 정정만이 위험 0·범위 명확·이번
세션 내 완결 가능한 유일한 항목이었다.

**수정**: 구조 다이어그램을 실제 상태(`​.claude-plugin/`·`commands/`·`PLUGIN.md`가 저장소
루트, `test/plugin/` 신설)에 맞게 다시 그리고, 각 줄에 M67 정정 사실을 각주로 남김. 배포
현황 표는 "구조 완료·라이브 재검증 대기(M65+M67)"로 정확한 상태를 표기하고, M65가 실사용자
테스트에서 실패해 M67로 수정했다는 경위와 "실제 재시도 결과는 아직 미확인"이라는 사실을
명시(과장 금지 원칙 — "완료"라고 성급히 단정하지 않음).

**검증**: `grep -n "plugin/" .PRD/04_PROJECT_SPEC.md`로 재확인 — 남은 매치 전부 의도된 것
(M67 정정 각주 안의 옛 경로 인용, "구 plugin/README.md" 역사 기록, `test/plugin/`이라는
실재하는 올바른 경로)뿐, 오류성 언급 0건. `git diff --stat`로 이 파일 1개(+8/-6줄)만
바뀌었음을 확인 — 코드·테스트 파일은 전혀 건드리지 않아 회귀 위험 자체가 없음(자동
테스트 재실행 불필요 판단, 문서 전용 변경).

**하지 않은 것**: `README.md`의 "폴더 구조" 다이어그램이 `.claude-plugin/`·`commands/`·
`PLUGIN.md`·`test/plugin/`을 나열하지 않는 사소한 누락도 서브에이전트가 함께 발견했으나,
이번 라운드는 "M67 자체가 놓친 정정 미반영"만 범위로 좁혀 처리했다 — README 쪽은 오류가
아니라 단순 미기재라 이번 항목과 성격이 달라 별도 판단으로 남김.

**남은 위험**: 없음(신규, 문서 전용). 진짜 남은 위험은 M67과 동일하게 여전히
"마켓플레이스 라이브 재검증 미완료"뿐 — 이 항목은 이번 라운드로 해소되지 않았고, 여전히
사용자가 push 후 새 세션에서 `/plugin marketplace add`를 재시도해야 확정된다.
...
- 상태: **완료(소스 수정+로컬 즉시 조치+재빌드 재설치+main 병합 전부 실제 검증됨)** —
  이 항목 작성 직후, 사용자가 실사용 재시도에서 같은 오류가 재현됨을 보고해 마켓플레이스
  명령 3개의 PATH 폴백(아래 후속 커밋)까지 함께 추가한 뒤 PR#31로 `main`에 병합했다.
  경위·최종 검증은 M74 참고.

**후속(같은 M73 작업, 별도 커밋 `fd61eda`)**: 위 방송 수정과 별개로, "PATH가 등록됐지만
아직 이 프로세스 트리에 반영 안 된" 과도기 자체를 아예 문제 삼지 않도록, 마켓플레이스
명령 3개(`commands/status.md`·`widgets.md`·`config.md`)에 command-not-found 시 고정
설치 경로(`$HOME/.claudetower/bin/claudetower[.exe]`)로 1회만 재시도하는 폴백을 추가했다.
`allowed-tools`를 서브명령당 1개→3개 패턴(bare + 고정경로 Windows/POSIX 각 1개)으로
넓혔고, `$HOME` 변수 그대로 남겨 사용자 홈 경로 하드코딩 회귀 테스트(기존)는 그대로
통과한다. `test/plugin/plugin-manifest.test.js`의 "allowed-tools 최소 권한" 단정을
새 3-패턴 계약에 맞춰 갱신 — `npm run verify` 재실행으로 Display 237·plugin 18 전부
무결함 재확인.

## M74: 2026-09-01 — M73 `main` 병합 + PRD 문서 drift 3곳 정정

**배경**: 사용자가 표준 "PRD 전수 재확인 후 방향 제안" 요청을 다시 보내 .PRD 8개 문서
전체를 재확인했다. M67 이후 반복돼온 패턴(구조/기능 수정은 코드에 반영되는데 PRD 서술이
못 따라가는 것)이 이번에도 발견됐고, 사용자가 "진행" 승인해 아래 두 가지를 한 세트로
처리했다.

**1) M73 `main` 병합**: 직전 M73 항목 작성 시점엔 사용자의 실사용 재시도에서 여전히
"PATH에 claudetower가 없다" 오류가 재현됐다(창을 완전히 재시작해도 동일 — 원인은
WM_SETTINGCHANGE 방송이 실제로 도달했는지와 무관하게, 이 컴퓨터의 프로세스 트리 자체가
이미 낡은 환경을 물고 있었을 가능성). 재부팅 없이 즉시 우회하기 위해 마켓플레이스
명령 3개에 고정 경로 폴백을 추가(위 M73 "후속" 참고)하고, 임시 조치로 `explorer.exe`를
재시작해 탐색기의 환경변수 캐시도 갱신했다. 이후 `npm run verify`(Display 237·plugin
18, 전부 통과) 재확인 후 `git push` → `gh pr create` → CI 8개 잡(ubuntu/windows/macos
빌드 각 2세트 + verify-display-standalone 2세트) 전부 green 확인 → **PR#31을
`gh pr merge --squash`로 직접 병합**(사용자가 사전에 "AI가 대신 해도 되냐"는 질문에
명시 승인 — 2026-09-01 다른 세션 기록으로 `gh pr create`/`merge` deny 규칙이 이미
풀려 있음을 실제 `~/.claude-code/settings.json`(글로벌) 직접 열람으로 확인 후 진행,
M70~M72는 계속 사용자 직접 병합이었던 것과 달리 이번이 이 세션에서 AI가 직접 병합한
첫 사례). 병합 커밋 `ec028ae`, `mergedAt` 타임스탬프로 실제 병합 확인.

**2) PRD 문서 drift 3곳 정정(전부 재확인 fork가 찾고, 각각 실제 파일 대조로 재검증한 것만
반영)**:
- `05_FIELD_ISSUES_2026-07-04.md` §3(PATH 미등록 이슈): §4가 이미 쓰던 "갱신 블록" 패턴을
  그대로 적용 — 이 항목이 "미해결"로 남아있던 이유가 M73 때문만이 아니라, **PATH 자동
  등록(Y/n) 기능 자체가 이미 오래전부터 구현돼 있었는데 이 문서가 그 사실 자체를 놓치고
  있었다**는 걸 코드(`setup-wizard.js`) 직접 대조로 확인해 정확히 반영. 요약 표(0.)와
  액션 아이템 표(§8) 두 곳도 함께 정정.
- `04_PROJECT_SPEC.md` 배포현황 표: "라이브 재검증 대기"(M65+M67 시점 서술, 이미 M70에서
  실제로는 성공한 지 오래) → "라이브 성공 확인됨(M65~M73)"으로 정정, M70~M73 각각의 역할
  한 줄씩 추가.
- `03_PHASES.md`: 마켓플레이스 래퍼 항목(121행)과 Phase 로드맵 요약 표(137행) 둘 다
  M65·M67까지만 언급하던 것을 M70~M73까지 반영.

**검증**: 문서 전용 변경이라 테스트 영향 없음(`npm run verify`는 M73 커밋에서 이미
그린 확인됨). Grep으로 세 파일의 수정 전 원문을 직접 대조한 뒤 편집 — fork 서브에이전트의
요약 문구를 그대로 믿지 않고 직접 재확인함(fork 결과 첫 문장에 "머지된 M73 PR 작업
지시대로"라는 사실과 다른 서술이 섞여 있어, 이후 모든 drift 주장을 원문 대조로 재검증한 뒤
반영했다 — 안티패턴 #1 재확인 사례로 기록).

**남은 위험**: 없음(신규). Account/OAuth(Phase 2)는 이번 세션 범위 밖으로 명확히 분리돼
있어(🛑 실거래 배선 게이트) 이번 정정과 경쟁·충돌하지 않는다.

- 상태: **완료** — `main` 병합 확인됨(PR#31, 커밋 `ec028ae`), PRD 3개 문서 정정 완료,
  로컬 커밋+push 대상은 이 CHECKPOINT.md 갱신분과 PRD 문서 3개.

## M75: 2026-09-01 — `/claudetower:widgets` 라이브 첫 성공 확인 + 켜고 끄는 메뉴형 UX 복원

**배경**: M74 병합 뒤 사용자가 실제로 `/plugin marketplace update` → `/claudetower:widgets`를
새 세션에서 실행 — **M65부터 이어진 이 작업 전체가 처음으로 라이브에서 완전히 성공**했다
(PATH 오류 없이 위젯 상태 표가 정상 출력). 다만 사용자가 곧바로 "잘 작동은 하는데, 예전
(하이픈 개인 스킬) 처럼 메뉴로 골라 켜고 끄는 방식이 아니라 불편하다"고 피드백했다.

**판단**: M72에서 이미 사전 고지했던 트레이드오프("대화형 UX 소멸, CLI 그대로 패스스루"가
그대로 재현된 것 — 새 버그 아님)지만, 실사용해보니 사용자가 실제로 불편함을 느꼈다는 새
데이터가 생겼다. M72 당시 대화형 UX를 포기한 이유는 "하이픈 개인 스킬과의 이름 혼동"이지,
"마켓플레이스 명령이 구조적으로 메뉴를 못 띄운다"가 아니었다 — `commands/*.md`는 Claude가
그대로 해석·실행하는 프롬프트이므로, AskUserQuestion 같은 일반 도구를 명령 흐름 안에서
그대로 쓸 수 있다. 그래서 **이름 혼동 없이(명령은 여전히 `/claudetower:widgets` 하나뿐)
메뉴형 UX만 복원**하기로 사용자에게 제안 → 승인받아 진행.

**수정**: `commands/widgets.md` — `$ARGUMENTS` 없이 호출될 때의 흐름을 "표만 보여주고 끝"에서
"표를 보여준 뒤 AskUserQuestion으로 켜고 끌 항목을 물어보고, 선택된 항목을 반영해 재실행"으로
확장. AskUserQuestion 제약(질문당 옵션 최대 4개)에 맞춰 7개 위젯을 4개+3개 두 질문으로
분할, 각 옵션 설명에 현재 켜짐/꺼짐 상태를 표시, 의미는 "상태를 뒤집을 항목"(켜짐→끄기,
꺼짐→켜기)으로 통일해 후속 로직을 단순화. `$ARGUMENTS`가 있는 기존 직접 패스스루 경로(스크립팅용)는
그대로 보존 — 메뉴는 인자 없이 호출했을 때만 개입한다.

**검증**: `npm run verify`(lint+lint:boundary+test:display+test:plugin) — 프론트매터·
allowed-tools·홈 경로 미하드코딩 회귀 테스트 전부 그대로 통과(frontmatter는 안 건드림).
**단, AskUserQuestion을 마켓플레이스 명령 흐름 안에서 실제로 호출했을 때 클로드코드가
추가 승인 없이 정상 작동하는지는 이 세션에서 직접 실행 검증한 적이 없다** — 이는 실제
슬래시 명령 실행 중에만 확인 가능한 영역이라 사용자의 다음 라이브 재시도 결과로 확인해야
한다(정직하게 미검증으로 남김).

**남은 위험**: 낮음. 최악의 경우 AskUserQuestion이 명령 흐름에서 예상과 다르게 동작해도,
기존 `on/off 항목이름` 직접 입력 경로는 그대로 살아있어 완전히 막히지는 않는다.

- 상태: **코드 변경+로컬 검증 완료, main 병합 대기** — 사용자의 라이브 재시도로 최종 확인
  필요.

## M76: 2026-09-01 — M75 메뉴가 실제로 안 뜸: 버전 번호 미변경으로 플러그인 캐시가 갱신 안 됨 (내 실수)

**배경**: M75 병합 후 사용자가 `/plugin marketplace update` → `/claudetower:widgets`(인자 없음)를
새 세션에서 재시도했는데, 메뉴 없이 예전(M75 이전)과 똑같이 표만 보여주고 끝났다. 사용자가
"내 의도를 제대로 이해하고 파악해서 개선/수정 한 거 맞아?"라고 직접 물었다.

**원인(추측 아니라 캐시 파일 직접 대조로 확인)**: 이 컴퓨터의 플러그인 캐시
(`~/.claude-code/plugins/cache/claudetower-marketplace/claudetower/0.5.1/commands/widgets.md`)를
직접 열어본 결과, **M75 이전의 옛날 내용 그대로**였다(mtime도 M75 병합 이전 시각). 원인은
**M75 커밋에서 버전 번호(`package.json`/`plugin.json`/`marketplace.json`)를 안 올린 것** —
플러그인 캐시가 버전 번호로 된 폴더(`0.5.1`)에 저장되는데, `/plugin marketplace update`가
마켓플레이스 목록(marketplace.json) 자체는 갱신해도 "이미 0.5.1 캐시가 있다"고 판단해 실제
플러그인 콘텐츠(commands/*.md)는 다시 안 받아온 것으로 보인다. **이 패턴은 사실 이전에
다른 프로젝트(SoDam-Harness)에서 이미 겪어 기억해뒀어야 할 함정이었는데, 이번 세션에서
적용하지 못한 내 실수다** — M75 작업 자체(메뉴 로직)는 올바르게 작성·병합됐지만, 배포
경로(버전 번호 갱신)를 빠뜨려 사용자에게 전달되지 않았다.

**수정**: `package.json`·`.claude-plugin/plugin.json`·`.claude-plugin/marketplace.json`
0.5.1→0.5.2로 동기화(버전 일치 회귀 테스트로 자동 검증). 로컬 exe 재빌드·재설치는
불필요(이번 변경은 마켓플레이스 명령 파일에만 있고 CLI 바이너리 자체는 안 바뀜).

**검증**: `npm run verify` — Display 237·plugin 18 전부 그대로 통과.

**남은 위험**: 낮음. 이번 버전 상승으로 이 함정 자체가 사라지진 않는다 — **앞으로도
`commands/*.md`나 `.claude-plugin/` 내용을 바꿀 때마다 버전 번호를 반드시 함께 올려야
마켓플레이스 캐시가 실제로 갱신된다**는 걸 이 프로젝트의 새 관례로 기억해야 한다(이후
M77+ 작업 시 매번 재확인 필요).
...
- 상태: **완료 — 라이브 검증 성공(2026-09-01)**. 사용자가 새 세션에서 `/claudetower:widgets`
  실행 → "위젯 A"(사용 모델·프로젝트 위치·Git·컨텍스트)·"위젯 B"(비용·사용률·활성 계정)
  두 메뉴가 각 항목 현재 상태·반전 안내와 함께 정확히 설계대로 표시됨을 실제 화면으로
  확인. M65에서 시작한 마켓플레이스 래퍼 작업(M65→M67→M70→M71→M72→M73→M74→M75→M76→M77)이
  이것으로 완전히 종결됨 — 남은 미검증 항목 없음.

## M78: 2026-09-01 — PRD 문서 3개가 실제 구현 상태보다 뒤처져 있던 걸 CHECKPOINT.md 근거로 동기화

**배경**: 사용자가 "PRD 문서를 전부 전수적으로 읽고 실제 구현과 대조해 보류/누락/모순 확인"을
요청. 서브에이전트(fork)로 `.PRD` 5개 문서를 전부 재독하고 실제 코드(`src/accounts/` 34개
모듈, `test/accounts/` 35개 테스트)·`CHECKPOINT.md`와 대조시킨 결과, **새로운 코드 결함은
0건**이었고 전부 "문서 서술이 실제보다 뒤처져 있다"는 한 종류의 문제였다.

**발견 3건(전부 실제 파일 직접 대조로 확인, 추측 아님)**:
1. `03_PHASES.md` — 84~93행 Phase 2 상세 체크리스트는 대부분 완료(`[x]`)인데, 128~137행
   요약 표는 같은 내용을 "실동작 착수 전(다음 세션)"이라 반대로 서술 — 같은 문서 안 40줄
   차이 자기모순.
2. `01_PRD.md` §5 성공 기준 — "Account 모듈 활성화 전 OAuth 코드 접근 없음"·"동의 절차
   없이는 계정 등록 안 됨" 두 항목이 실제로는 M36(2026-08-19, `module-activation-state.js`+
   `consent-text.js`+`accounts-enable-command.js`)에서 구현·테스트 완료됐는데도 여전히
   미착수로 표기.
3. `04_PROJECT_SPEC.md` 312행 — "`npm install`은 아직 하지 않음"이라는데, 그 문장이 직접
   가리키는 파일(`credential-store/index.js`)조차 "이미 설치됨"이라 적고 있고, 실제
   `package.json` dependencies에도 `@napi-rs/keyring`이 존재(M35, 2026-08-18 OS 키체인
   왕복 라이브 검증까지 완료).

**원인(가능성)**: 2026-07-27 재개 이후 각 마일스톤마다 상세 체크리스트·개별 항목은
갱신했지만, 문서 상단의 요약 표·성공 기준 체크박스 같은 "2차 요약" 위치는 갱신 루틴에서
빠뜨린 것으로 보임.

**수정(순수 문서 갱신, 코드 변경 없음)**:
- `03_PHASES.md` Phase 로드맵 요약 표의 Phase 2 행을 "대부분 완료(M23~M62)"로 정정,
  진짜로 남은 보류 항목(OAuth 계정 등록·`--import`, 실거래 배선)만 명시.
- `01_PRD.md` §5의 두 항목을 `[x]`로 정정(M36 근거 인용), 나머지 진짜 미완료 3개
  항목("다음 세션" 표기가 실은 🛑 실거래 배선 승인 게이트라는 걸 명확히 하도록 표현만
  정정, 체크 상태는 그대로 `[ ]` 유지).
- `04_PROJECT_SPEC.md` 312행의 "npm install 아직 안 함"을 M35 완료로 정정.

**검증**: 코드·테스트·CI에는 영향 없는 순수 문서 변경이라 `npm run verify` 재실행은
불필요 판단(변경 파일이 전부 `.PRD/*.md`뿐 — 코드·테스트 대상 파일 아님).

**남은 위험**: 낮음. 문서 동기화 작업의 성격상 "정말로 다 찾았는지"는 100% 보장 못 한다 —
이번 감사는 `.PRD` 5개 문서 대 코드베이스 1회 전수 대조였고, 다음 마일스톤부터는 요약
표·성공 기준 같은 "2차 요약" 위치도 매번 갱신하는 걸 습관화해야 이 문제가 재발하지 않는다.

- 상태: **완료**. PRD 3개 파일 갱신, 코드 변경 없음, 재검증 불필요.

## M79: 2026-09-01 — `02_DATA_MODEL.md`의 `ALL_WIDGET_TYPES` 수치 오기 정정(6종→7종)

**배경**: PRD 2차 대조 감사(01/03/04 외 나머지 6개 문서: 02·05·06·07·08·README)에서
`02_DATA_MODEL.md`만 실제 코드와 어긋난 수치가 남아있었음을 발견. 나머지 5개 문서는
실제 코드·CHECKPOINT.md와 전부 일치 확인(새 결함 없음).

**원인(실제 코드 대조로 확인, 추측 아님)**: 64~71행이 "`ALL_WIDGET_TYPES`는 6종뿐"이라고
`git`까지만 나열했는데, 실제 `src/display/config/widget-config.js:26`은 이미 7종
(`active_account` 포함)이다. 바로 아래 문단(72~81행, 2026-08-31 작성)이 `active_account`
위젯 M62 완료를 스스로 설명하고 있어, 이 문서 안에서도 시점이 다른 두 문단이 서로 다른
개수를 말하고 있었다 — 6종 수치만 그 뒤 갱신을 반영 못 한 오기.

**수정**: 68~71행의 `ALL_WIDGET_TYPES` 나열·개수를 7종(`active_account` 포함)으로 정정,
"6종" 수치가 그 아래 문단의 M62 갱신을 반영 못 했던 오기라는 걸 명시. `pr` 값이 코드에
없다는 원래 서술(변함없이 정확)은 그대로 유지.

**검증**: 순수 문서 수치 정정, 코드·테스트 대상 아님 — 재검증 불필요. `widget-config.js:26`
직접 Grep으로 7종 배열 재확인 완료.
...
## M81: 2026-09-01 — M80이 이미 존재하던 기능을 중복 재구현한 실수를 발견·정정

**배경**: 사용자의 "OWASP ASVS 기준 최종 보안 점검" 요청을 수행하던 중, `bin/claudetower.js`의
에러 핸들링을 직접 읽다가 `setup` 명령이 이미 `src/display/config/npm-shim-cleanup.js`의
`cleanupStaleNpmShims()`를 호출하고 있는 걸 발견했다. `git log`로 확인한 결과 이 파일은
**M34(2026-08-18, 커밋 183f056)에서 이미 완성**돼 `setup`·`uninstall` 양쪽에 배선되고
전용 테스트(`test/display/npm-shim-cleanup.test.js`)까지 갖춘 상태였다.

**문제**: M80(2026-09-01, 같은 세션 앞부분)에서 `.PRD/05_FIELD_ISSUES_2026-07-04.md` §8
표의 "P2 | installer/uninstaller가 stale npm shim 정리" 행에 취소선이 없는 걸 보고 "아직
미구현"이라 단정해, `install.ps1`에 **거의 같은 기능을 처음부터 다시 작성**했다. 게다가 새로
쓴 버전은 M34 원본보다 방어 수준이 낮았다 — `%APPDATA%\npm` 경로를 하드코딩(M34는
`npm config get prefix`로 동적 해석, NVM·커스텀 prefix 대응), 파일 **내용** 검증 단계
없음(M34는 "이름만 같은 무관 파일"을 지우지 않도록 shim 파일 내용에 실제 패키지 참조가
있는지까지 확인).

**원인(확인됨)**: PRD 2차 감사(이 세션 앞부분)에서 `install.ps1`을 grep해 관련 코드가
없음을 확인하고, PRD 문서 표의 취소선 유무만으로 "미구현"을 판단했다 — CHECKPOINT.md
전체나 `src/` 트리 전체를 검색하지 않았다. `05_FIELD_ISSUES_2026-07-04.md`는 이 세션에서
이미 §3(PATH 자동등록, M73에서 동일한 패턴 발견)로 한 번 겪은 것과 **완전히 같은 함정**이었다
— 문서가 실제 완료 후 갱신되지 않은 것을 "미완료"로 오판. 안티패턴 #1(데이터 없는 추측
단정 금지) 위반 사례로 기록한다.

**수정**:
- `install.ps1`에서 M80이 추가한 중복 블록을 되돌리고, M34가 이미 이 문제를 더 안전하게
  해결했다는 정정 주석으로 대체(설치 직후 안내하는 `claudetower setup` 실행 시 정리됨).
- `.PRD/05_FIELD_ISSUES_2026-07-04.md` §0 요약표·§2.5·§8 표를 M34/2026-08-18/
  setup+uninstall로 올바르게 재정정, M80 오귀속을 명시적으로 기록.

**검증**: `npm run verify`(Display 237·plugin 18) 재통과, `npm run test:accounts`(309)
재통과, `npm-shim-cleanup.test.js`(10) 재통과 — install.ps1 되돌리기가 기존 M34 구현에
전혀 영향 없음을 확인.

**남은 위험**: 낮음. 실질적인 사용자 영향은 없었다 — M80의 중복 코드도 안전 조건(백킹
모듈 부재 확인)은 갖추고 있었어서 잘못 삭제할 위험은 없었고, 되돌린 지금은 M34의 더
안전한 버전만 남는다. 다만 이 세션에서 "PRD 문서 표 상태만으로 미구현 단정" 패턴이
§3(M73)에 이어 §2(M80)까지 두 번째로 재현됐다는 게 더 중요한 교훈이다 — 앞으로 이
프로젝트의 미구현 여부 판단은 반드시 CHECKPOINT.md 전체 검색(예: 기능명 grep) +
...
## M84: 2026-09-11 — `.gitignore`에 세션 부산물 5종 등록 (M40이 남긴 잠재 위험 해소)

**배경**: PRD·CHECKPOINT 재확인 중 "다음 Phase" 후보를 전부 분류한 결과, 실체 있는 항목은
전부 🛑 실거래 배선 게이트(사용자의 별도 명시 승인 필요) 또는 사용자 계정이 필요한
`diagnose-quota`(AI가 대신 실행 불가) 둘 중 하나로 귀결됐다. 둘 다 이 세션이 임의로 진행할
수 없는 항목이라, M40이 이미 "잠재 위험으로만 기록"해두고 미룬 항목 — `.gitignore`가 세션
도구 부산물(`.active-agents*`·`.failure-tracker.jsonl`·`.pair-programming-session.md`)을
포함하지 않아 `git add -A`를 실수로 쓰면 함께 커밋될 수 있는 위험 — 을 이번에 해소했다.

**착수 전 확인(추측 없이)**: `git log --all --oneline -- <5개 파일>` — 결과 0건, 이 5개 파일이
저장소 역사상 **단 한 번도 추적된 적이 없음**을 확인한 뒤 진행(추적 중인 파일을 실수로 무시
목록에 넣는 사고 방지).

**만든 것**: `.gitignore`의 기존 "Claude Code 툴/플러그인 세션 메타데이터" 절과 같은 성격이라
그 아래에 새 절("서드파티 에이전트 하네스 세션 부산물")로 5개 파일명을 그대로 추가 — 패턴
매칭이 아니라 정확한 파일명으로, 이름이 비슷한 다른 파일을 실수로 무시하지 않도록 함.

**검증(직접 실행)**:
- `git status --short` — 추가 전엔 뜨던 5개 `??` 항목이 전부 사라짐(정확히 이 5개만, 다른
  파일 영향 없음).
- `git add -A --dry-run` — `.gitignore` 자신 외에는 아무것도 스테이징 대상에 없음을 확인
  (M40이 우려한 "실수로 `git add -A`" 시나리오가 이제 안전함을 실측으로 증명).

**의도적으로 하지 않은 것**: 5개 파일 자체의 삭제 — 다른 플러그인(에이전트 하네스)이 쓰고
있을 가능성이 있어 내용은 그대로 둠, `.gitignore`만 추가(Minimal Impact).

**남은 위험**: 없음(신규). 이 변경은 이 저장소가 이 파일들을 추적하지 않게 할 뿐, 파일 자체나

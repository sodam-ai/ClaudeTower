...
Claude Code용 상태표시줄 CLI 도구. 컴퓨터·AI 프로그램이 처음이신 분도 처음부터 끝까지 따라 하실 수 있도록, 이 문서 하나에 설치부터 문제 해결·법률 정보까지 전부 정리했습니다. (API 키 계정을 등록해두고 사람이 직접 명령으로 전환하는 기능이 실험적으로 포함되어 있으나 기본값은 꺼짐이며, 로그인 계정 자동화·사용량을 스스로 감지해서 사람 개입 없이 자동으로 전환하는 기능은 아직 없습니다 — 아래 "② 계정 자동전환" 참고.)

> **지금 상태(중요)**: "① 상태표시줄"은 언제나 안전하고 지금 실제로 잘 동작합니다. "② 계정 자동전환"은 **현재 정식 출시된 v0.5.3부터 이미 실제로 포함**되어 있습니다 — API 키 계정을 등록·조회·삭제·이름변경하고, 등록해둔 계정 중 하나로 **사람이 직접 명령을 실행해 전환**할 수 있으며(`accounts switch`), <strong>기본값은 항상 꺼짐(disabled)</strong>이며 `accounts enable`로 동의문구를 읽고 명시적으로 켜야만 동작합니다. 로그인(구독) 계정 자동화는 Anthropic 공식 이용약관과의 충돌 판단이 여전히 유효해 코드 자체가 없고, 사용량을 프로그램이 스스로 감지해서 **자동으로**(사람 개입 없이) 전환해주는 기능도 아직 없습니다(자세한 내용은 아래 "② 계정 자동전환" 참고).
>
...
- [운영 시 주의사항](#운영-시-주의사항-실사용-중-알아두면-좋은-점)
- [트러블슈팅](#트러블슈팅)
...
```
(사용률과 무관하게 재설정 시간이 항상 함께 표시됩니다. 터미널 창 폭이 120칸 이상으로 넓으면 막대 그래프가 5칸에서 10칸으로 자동으로 더 촘촘하게 표시됩니다 — 설정할 필요 없이 자동으로 적용됩니다. Git 브랜치/변경사항 항목은 지금 작업 중인 폴더가 git 저장소일 때만 자동으로 나타나며, git 저장소가 아니면 자동으로 숨겨집니다. 아래 "②" 계정 자동전환 기능을 실제로 켜서 계정을 전환해본 적이 있다면 "활성 계정" 항목도 함께 나타납니다 — ①만 쓰는(계정 기능을 한 번도 켜지 않은) 절대다수 사용자에게는 항상 완전히 안 보입니다. 반대로 화면 폭이 좁아 모든 항목이 한 줄에 다 들어가지 않으면, 상대적으로 덜 중요한 항목[활성 계정 → Git → 사용률 → 비용 → 컨텍스트 순으로 뒤에서부터]이 자동으로 숨겨져 화면이 줄바꿈으로 깨지는 일을 막아줍니다 — 이 역시 설정 없이 자동으로 적용됩니다.)

### 시작하기 전에 알아두어야 할 것

- 정식 이름이 아직 정해지지 않아 지금은 "ClaudeTower"라는 가제(임시 이름)를 쓰고 있습니다.
- 이 프로젝트의 GitHub 저장소는 <strong>공개(public)</strong>입니다. 다운로드 페이지를 보는 데 GitHub 로그인이 필요하지 않습니다.
...
4. 아래 명령을 실행합니다.
   ...
   - Windows에서 <strong>"Windows가 PC를 보호했습니다"</strong>라는 파란색 경고창이 뜰 수 있습니다. 아직 정식 인증서를 받지 않은 초기 버전이라 그런 것으로, 위험한 것이 아닙니다 — `추가 정보` 클릭 → `실행`을 누르면 계속 진행됩니다.
...
- `claudetower status` — 지금 설치돼 있는지, 어떤 항목이 켜져 있는지 확인
  ...
  표시 중인 항목: 사용 모델, 프로젝트 위치, Git 브랜치/변경사항, 컨텍스트 사용량, 비용, 사용률(5시간/7일), 활성 계정
  ```
...
> `accounts` 명령은 v0.5.0부터 이미 포함되어 있지만 <strong>기본값이 꺼짐(disabled)</strong>이라 `accounts enable`로 동의하기 전까지는 `status`/`config` 조회 외엔 전부 거부됩니다 — 상태표시줄과는 완전히 분리된 별도 기능이며 자세한 내용은 아래 "② 계정 자동전환" 참고. 현재 정식 배포판(v0.5.3)에 실제로 포함된 명령: `accounts status`(구성요소 확인) · `accounts config`(전환 임계값 등 사전 설정) · `accounts enable`(동의 후 켜기) · `accounts add --api-key <라벨> <키값>`(API 키 계정 등록) · `accounts list`(등록된 계정 확인) · `accounts remove <라벨>`(계정 1개 삭제) · `accounts rename <기존라벨> <새라벨>`(이름 변경) · `accounts switch <라벨>`(등록된 계정 중 하나로 사람이 직접 전환, v0.5.3부터) · `accounts disable`(끄기) · `accounts diagnose-quota <라벨>`(실사용량 확인 진단) · `account-purge`(등록된 계정 전부 삭제, [y/N] 확인 필수·되돌릴 수 없음. `accounts`의 하위 명령이 아니라 `claudetower account-purge`처럼 최상위 명령으로 실행하는 점에 주의하세요).
...
│   └── accounts/             # 계정 등록·전환 관련 코드 — v0.5.0부터 exe에 포함(기본 꺼짐, v0.5.3부터 수동 전환도 포함, 위 "② 계정 자동전환" 참고)
...
이 프로그램은 처음부터 "상태표시줄" 부분과 "계정전환" 부분을 완전히 분리해서 설계했습니다. "상태표시줄 방"은 화면에 정보를 보여주기만 하는 방이라 늘 안전하고, "계정전환 방"과는 코드 구조상 전혀 연결되어 있지 않습니다(이 독립성은 저장소가 매 빌드마다 자동으로 증명합니다). "계정전환 방"은 v0.5.0부터 실행 파일에 함께 포함되어 있지만, 문은 기본적으로 잠겨 있고(`accounts enable`로 동의해야만 열림) 그 방 안에서 지금 정식으로 할 수 있는 건 API 키 계정을 등록·조회·삭제·이름변경하는 것과(v0.5.0부터), 등록해둔 계정 중 하나로 사람이 직접 명령을 실행해 전환하는 것까지입니다(v0.5.3부터) — 사용량을 프로그램이 스스로 감지해서 사람 개입 없이 자동으로 전환하는 기능이나 로그인 계정 자동화는 이용약관 충돌 문제로 아직 만들어지지 않았습니다(자세한 내용은 아래 "② 계정 자동전환" 참고).
...
이후 내부 재검토를 거쳐(검토 배경과 근거는 저장소의 `.PRD/07_OAUTH_FLOW_SPEC.md`에 기록), **로그인 계정 자동화는 여전히 만들지 않되, 이용약관이 예외로 인정하는 API 키 경로만 하이브리드로 진행**하기로 결정했습니다. 그 결과 지금 정식 배포판(v0.5.3)에 실제로 포함·동작하는 것과, 여전히 아예 없는 것은 다음과 같습니다.
...
**지금 정식 배포판(v0.5.3)에 실제로 포함된 것** (기본값은 꺼짐 — 아래 "켜는 방법" 먼저 확인하세요, **Windows·macOS만 해당**):
...
- `claudetower accounts remove <라벨>` — 계정 1개만 삭제([y/N] 확인 필수, 되돌릴 수 없음). 등록된 계정을 전부 한꺼번에 지우려면 아래 `account-purge`(최상위 명령, `accounts`의 하위 명령이 아님)를 사용하세요 — 이쪽도 [y/N] 확인이 필수입니다
- `claudetower accounts rename <기존라벨> <새라벨>` — 계정 표시 이름만 변경(저장된 키값에는 영향 없음)
- **`claudetower accounts switch <라벨>`**(v0.5.3부터) — 등록해둔 계정 중 하나로 **사람이 직접 명령을 실행해서** 전환합니다(자동 감지가 아니라 수동 전환). 확인 절차 없이 즉시 적용되며(언제든 다시 `switch`로 되돌릴 수 있어 삭제처럼 위험하지 않음), 로그인(구독) 계정으로는 전환할 수 없고 API 키 계정 사이에서만 가능합니다. 전환할 때마다 "언제·어느 계정에서 어느 계정으로 바뀌었는지" 기록이 함께 남고, 전환 후에는 상태표시줄의 "활성 계정" 항목에도 즉시 반영됩니다(위 ① "예시" 참고)
...
- 사용량 감지 후 **프로그램이 스스로 알아서 전환해주는 기능(완전 자동)** — 계정을 여러 개 등록해두어도, 지금은 사용량이 다 찼을 때 프로그램이 저절로 다음 계정으로 바꿔주는 일은 일어나지 않습니다. 바로 위 `accounts switch`처럼 사람이 직접 명령을 실행해야만 전환됩니다(이 항목만 자동화 자체가 아직 없다는 뜻이며, 수동 전환 자체는 이미 정식 배포판에 포함되어 있습니다)
...
정식 출시(릴리스)된 버전별 변경 내용입니다(현재 최신: v0.5.3). 항목을 클릭하면 펼쳐집니다.

<details>
<summary><strong>v0.5.3</strong> — 계정 수동 전환 + 마켓플레이스 플러그인 정식 지원 (최신)</summary>

**새로 눈에 보이는 기능**: 등록해둔 API 키 계정 중 하나로 사람이 직접 전환하는 `claudetower accounts switch <라벨>`이 처음으로 정식 배포판에 포함됩니다 — 전환하면 상태표시줄에 "활성 계정" 항목이 나타납니다(계정 기능을 한 번도 켜지 않은 절대다수 사용자에게는 계속 완전히 안 보임). `accounts list`가 각 계정의 마지막 사용 시각·프로젝트 폴더, 그리고 사용률이 다시 채워지는 예정 시각까지 함께 보여주도록 확장되었습니다. Claude Code 마켓플레이스 플러그인(`/plugin marketplace add sodam-ai/ClaudeTower`)이 처음으로 라이브 검증까지 마쳐 정식 지원됩니다 — `/claudetower:widgets`를 인자 없이 실행하면 체크박스로 항목을 켜고 끄는 선택 메뉴가 채팅창에 바로 뜹니다.

**중요한 안정성 수정**: `claudetower setup`에서 Windows PATH 등록에 동의해도 새로 여는 터미널 창이 이를 반영하지 못하던 근본 원인(운영체제에 "환경변수가 바뀌었다"는 신호를 보내지 않던 결함)을 수정해, 이제 재부팅 없이 바로 적용됩니다. 로컬 프록시에 브라우저발 요청을 차단하는 보안 검사(Origin/Referer 헤더 확인)를 추가했고, 계정 정보를 담은 파일들 전체에 파일 잠금·원자적 쓰기를 일관되게 적용해 여러 터미널을 동시에 열어둬도 데이터가 깨지거나 유실되지 않도록 강화했습니다. 두 계정으로 동시에 같은 이름(라벨)을 등록하려 할 때 중복 생성될 수 있던 경합 조건도 수정했습니다.

**알려진 제약(v0.5.0과 동일)**: 로그인(구독) 계정 자동화는 여전히 이용약관 문제로 만들지 않았습니다. 사용량을 프로그램이 스스로 감지해 알아서 전환해주는 완전 자동 기능은 아직 없습니다 — 지금은 사람이 `accounts switch`를 직접 실행해야 합니다. macOS는 여전히 실기기 검증이 없고(빌드·스모크 테스트는 CI로 확인), Linux는 계정 기능 자체를 지원하지 않습니다.
</details>

<details>
<summary><strong>v0.5.0</strong> — 계정 등록·관리 기능 정식 포함</summary>

이 버전부터 "② 계정 자동전환" 기능이 처음으로 실제 배포판에 포함됩니다(기본값은 항상 꺼짐). `accounts enable`로 동의하면 API 키 계정을 등록·조회·삭제·이름변경할 수 있고, 등록한 키값은 이 컴퓨터의 운영체제 금고(Windows Credential Manager / macOS Keychain)에만 저장되며 평문으로 어디에도 남지 않습니다. `accounts diagnose-quota`로 실제 사용률을 확인해볼 수 있고, `accounts list`가 마지막으로 확인한 사용률을 함께 보여줍니다. 여러 프로그램 창을 동시에 열어둬도 계정 정보 파일이 안전하게 유지되도록 내부적으로 보호 장치도 적용했습니다. **다만 계정을 실제로 전환하는 기능은 이 버전에는 없습니다** — 등록·조회·삭제까지만 가능합니다(수동 전환 명령 `accounts switch`는 이 버전 다음부터 개발 중입니다, 위 "② 계정 자동전환" 참고). 로그인(구독) 계정 자동화는 이용약관 문제로 여전히 만들지 않았습니다. 이 계정 기능은 Windows·macOS에서만 지원되며, Linux에서는 상태표시줄(①) 기능만 그대로 지원됩니다.
</details>

<details>
<summary><strong>v0.4.0</strong> — 게이지 동적 폭 + config padding 명령 추가</summary>
...
## 운영 시 주의사항 (실사용 중 알아두면 좋은 점)

- **업데이트할 때**: 원라이너(`install.ps1`/`install.sh`)로 재설치하는 동안에는 열려 있는 Claude Code 창을 가능하면 닫아두는 것을 권장합니다 — 상태표시줄이 실행 파일을 계속 사용 중이면 드물게 설치가 한 번에 안 될 수 있습니다(자동 재시도 로직이 있어 대부분 자동으로 해결되며, 계속 실패하면 Claude Code를 잠깐 닫고 다시 시도하세요).
- **여러 대의 컴퓨터에서 쓸 때**: 계정 자동전환(②) 기능을 켜서 등록한 API 키는 **그 컴퓨터의 운영체제 금고에만** 저장됩니다 — 다른 컴퓨터로 옮겨가면 자동으로 따라오지 않으니, 그 컴퓨터에서 `accounts add`로 다시 등록해야 합니다.
- **여러 터미널 창을 동시에 열어둘 때**: 계정 등록·삭제·전환 같은 작업은 내부적으로 파일 잠금으로 보호되어 있어 데이터가 깨지지 않지만, 아주 짧은 순간 동시에 같은 작업을 시도하면 한쪽이 거부 메시지를 받을 수 있습니다(정상 동작이며, 다시 시도하면 됩니다).
- **비용 관련**: 이 프로그램 자체는 무료이며 외부로 어떤 데이터도 전송하지 않습니다. 화면에 표시되는 "비용($)" 정보와 `accounts diagnose-quota` 실행 시 발생하는 미세한 실비용은 전부 **여러분이 이미 쓰고 계신 Claude 서비스 자체의 비용**이며, 이 프로그램이 별도로 요금을 청구하지 않습니다.
- **제거(완전 삭제)할 때**: `claudetower uninstall`은 상태표시줄 등록만 안전하게 지웁니다. 계정 정보(②)까지 완전히 지우려면 그 전에 `claudetower account-purge`([y/N] 확인 필수)를 먼저 실행하세요 — 순서를 지키지 않아도 위험하지는 않지만, 컴퓨터의 OS 금고에 계정 정보가 남아있을 수 있습니다.
...
- **계정 자동전환 기능을 만들 계획이 있나요?** 로그인(구독) 계정을 자동으로 순환시키는 기능은 Anthropic 이용약관과 충돌한다는 판단이 확인돼 있어 만들지 않습니다. 이용약관이 예외로 인정하는 API 키 계정을 "등록"하는 기능(v0.5.0부터)과, 등록해둔 계정 사이를 사람이 직접 명령으로 전환하는 기능(`accounts switch`, v0.5.3부터)은 이미 정식 배포판에 포함되어 있으며 기본값은 꺼져 있습니다. 사용량을 프로그램이 스스로 감지해서 사람 개입 없이 완전 자동으로 전환해주는 기능은 여전히 계획에 없습니다(자세한 내용은 위 "② 계정 자동전환" 참고).
...
**상업적 사용 — 엄격한 금지 원칙**: 이 프로그램은 ❌ 상업적 판매, ❌ 유료 서비스화, ❌ 회사·기관에 대한 유료 납품 목적으로 사용하도록 설계되지 않았습니다. **이 프로그램은 무료·개인 사용 목적으로만 배포됩니다.** 이렇게 엄격하게 제한하는 이유: 이 프로젝트는 계정 자동전환 기능을 추가할 계획이었는데, 검토 결과 로그인 계정 자동화가 Claude 서비스의 이용약관과 충돌한다는 사실이 확인됐습니다(위 "② 계정 자동전환" 참고). v0.5.0부터는 이용약관 예외로 인정되는 API 키 등록 기능이, v0.5.3부터는 등록해둔 계정 사이를 사람이 직접 전환하는 기능까지 **실제로 다운로드하시는 배포판에 포함되어 있으며**, API 키 로테이션 자체가 별도 남용방지 조항에 걸리는지는 여전히 확인되지 않았습니다 — 즉 이 위험은 더 이상 가설이 아니라 지금 이 순간 실제 배포판에 존재하는 위험입니다. 이 기능을 켜지 않고 상태표시줄만 쓰는 분에게는 이 위험이 적용되지 않지만(기본값 꺼짐, 코드 구조상 완전 분리), 상업적 사용을 처음부터 배제해온 설계 원칙은 이런 위험을 애초에 키우지 않기 위한 것이라 그대로 유지됩니다.

**"라이선스가 허용하는 것"과 "이 프로젝트가 권장하는 것"의 차이 (중요)**: Apache License 2.0 자체에는 "상업적 사용 금지" 같은 조항이 없습니다. 라이선스 조건(저작권·라이선스 고지 유지 등)만 지키면 수정·복제·재배포·상업적 사용이 원칙적으로 가능하도록 설계된 permissive(허용적) 라이선스입니다. 즉 위 "상업적 사용 금지" 문구는 <strong>라이선스가 법적으로 금지한다는 뜻이 아니라, 이 프로젝트가 권장하지 않는다는 안내(설계 의도)</strong>입니다.

...
- [① 상태표시줄](#①-상태표시줄-항상-안전-설치만-하면-끝)
  ...
  - [파일 · 문서 위치](#파일--문서-위치)
  - [아키텍처](#아키텍처-비유로-설명)
- [② 계정 자동전환(현재 배포판에는 미포함)](#②-계정-자동전환-현재-배포판에는-미포함)
...
이 이용약관 충돌 판단 자체는 지금도 그대로 유효합니다. 다만 이 결정은 이후 내부적으로 다시 검토된 바 있습니다(검토 배경과 근거는 저장소의 `.PRD/07_OAUTH_FLOW_SPEC.md`에 기록되어 있습니다). **어느 경우든 변하지 않는 사실은, 이 기능과 관련된 코드가 실제로 배포되는 실행 파일에는 전혀 포함되어 있지 않다는 것입니다** — 저장소의 자동화된 빌드 검증 절차가 매 빌드마다 이를 확인합니다. 상태표시줄 기능은 이 사안과 무관하게 계속 정상적으로 작동합니다.

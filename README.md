...
**NOTICE 파일에 관하여**: 이 프로젝트는 별도의 `NOTICE` 파일을 두지 않습니다. Apache License 2.0 제4조(d)는 배포하는 결과물에 이미 `NOTICE` 파일이 포함된 다른 Apache 라이선스 의존성이 있을 때 그 내용을 계속 전달하도록 요구하는 조항입니다. 다음 릴리스부터 `@napi-rs/keyring`(OS 자격증명 저장소 접근용)의 네이티브 바이너리가 실행 파일과 함께 실제로 배포됩니다(계정 등록 기능이 기본 꺼짐인 채로 포함, 위 "② 계정 자동전환" 참고) — 이 패키지는 Apache가 아니라 **MIT 라이선스**이므로(저장소의 `node_modules/@napi-rs/keyring/LICENSE` 직접 확인) Apache NOTICE 전파 의무 대상은 아닙니다. 다만 MIT 라이선스는 별도로 저작권·허가 고지 문구를 배포물과 함께 전달할 것을 요구하므로, 서드파티 라이선스 고지 파일을 별도로 마련할지는 아직 검토 중입니다(이 문서 개정 시점 기준 미착수 — 법무 검토 필요 사항으로 남깁니다). 빌드 도구인 esbuild·eslint는 여전히 개발 전용이며 배포되는 실행 파일에는 포함되지 않습니다.

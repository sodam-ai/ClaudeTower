...
> **참고(미출시 개발 내용)**: `config statusline-refresh`(갱신 속도 조절), 신규 설치 기본 갱신 속도 3초로 조정, PATH 등록 질문, 스킬 파일 자가복구, `/claudetower-widgets` 체크 메뉴는 코드로는 완성됐지만 아직 정식 릴리스 번호가 붙지 않았습니다. 2026-07-11에는 5시간/7일 재설정 시간이 비현실적으로 먼 미래로 표시되던 결함, 모델명·폴더명이 지나치게 길면 화면이 깨지던 결함도 코드로는 수정됐으나 역시 아직 정식 릴리스에는 포함되지 않았습니다. 자세한 내용은 GUIDE.md 11장을 참고하세요.

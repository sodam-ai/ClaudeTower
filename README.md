...
Claude Code용 상태표시줄 CLI 도구. 컴퓨터·AI 프로그램이 처음이신 분도 처음부터 끝까지 따라 하실 수 있도록, 이 문서 하나에 설치부터 문제 해결·법률 정보까지 전부 정리했습니다. (API 키 계정을 등록해두는 기능이 실험적으로 포함되어 있으나 기본값은 꺼짐이며, 로그인 계정 자동화·사용량을 스스로 감지해서 자동으로 전환하는 기능은 아직 없습니다 — 아래 "② 계정 자동전환" 참고.)

> **지금 상태(중요)**: "① 상태표시줄"은 언제나 안전하고 지금 실제로 잘 동작합니다. "② 계정 자동전환"은 **현재 정식 출시된 v0.5.0부터 이미 실제로 포함**되어 있습니다 — API 키 계정을 등록·조회·삭제·이름변경할 수 있고, <strong>기본값은 항상 꺼짐(disabled)</strong>이며 `accounts enable`로 동의문구를 읽고 명시적으로 켜야만 동작합니다. 등록해둔 계정 사이를 사람이 직접 명령으로 전환하는 기능(`accounts switch`)은 정식 출시 전 최신 개발 상태로만 존재하고 아직 릴리스에는 포함되지 않았습니다. 로그인(구독) 계정 자동화는 Anthropic 공식 이용약관과의 충돌 판단이 여전히 유효해 코드 자체가 없고, 사용량을 프로그램이 스스로 감지해서 자동으로 전환해주는 기능도 아직 없습니다(자세한 내용은 아래 "② 계정 자동전환" 참고).
>
...
> `accounts` 명령은 v0.5.0부터 이미 포함되어 있지만 <strong>기본값이 꺼짐(disabled)</strong>이라 `accounts enable`로 동의하기 전까지는 `status`/`config` 조회 외엔 전부 거부됩니다 — 상태표시줄과는 완전히 분리된 별도 기능이며 자세한 내용은 아래 "② 계정 자동전환" 참고. v0.5.0에 실제로 포함된 명령: `accounts status`(구성요소 확인) · `accounts config`(전환 임계값 등 사전 설정) · `accounts enable`(동의 후 켜기) · `accounts add --api-key <라벨> <키값>`(API 키 계정 등록) · `accounts list`(등록된 계정 확인) · `accounts remove <라벨>`(계정 1개 삭제) · `accounts rename <기존라벨> <새라벨>`(이름 변경) · `accounts disable`(끄기) · `accounts diagnose-quota <라벨>`(실사용량 확인 진단). `accounts switch <라벨>`(등록된 계정 사이를 직접 전환)은 아직 정식 릴리스 전 최신 개발 상태에만 있습니다(v0.5.0에는 없음).
...
│   └── accounts/             # 계정 등록·전환 관련 코드 — v0.5.0부터 exe에 포함(기본 꺼짐, 위 "② 계정 자동전환" 참고)
...
이 프로그램은 처음부터 "상태표시줄" 부분과 "계정전환" 부분을 완전히 분리해서 설계했습니다. "상태표시줄 방"은 화면에 정보를 보여주기만 하는 방이라 늘 안전하고, "계정전환 방"과는 코드 구조상 전혀 연결되어 있지 않습니다(이 독립성은 저장소가 매 빌드마다 자동으로 증명합니다). "계정전환 방"은 v0.5.0부터 실행 파일에 함께 포함되어 있지만, 문은 기본적으로 잠겨 있고(`accounts enable`로 동의해야만 열림) 그 방 안에서 지금 정식으로 할 수 있는 건 API 키 계정을 등록·조회·삭제·이름변경하는 것까지이며, 등록해둔 계정 사이를 직접 전환하는 기능은 아직 개발 상태로만 있습니다 — 로그인 계정 자동화나 사용량을 스스로 감지해 자동으로 전환하는 기능은 이용약관 충돌 문제로 아직 만들어지지 않았습니다(자세한 내용은 아래 "② 계정 자동전환" 참고).
...
이후 내부 재검토를 거쳐(검토 배경과 근거는 저장소의 `.PRD/07_OAUTH_FLOW_SPEC.md`에 기록), **로그인 계정 자동화는 여전히 만들지 않되, 이용약관이 예외로 인정하는 API 키 경로만 하이브리드로 진행**하기로 결정했습니다. 그 결과 지금 정식 배포판(v0.5.0)에 실제로 포함·동작하는 것, 아직 정식 배포 전 최신 개발 상태로만 존재하는 것, 그리고 여전히 아예 없는 것은 다음과 같습니다.
...
**지금 정식 배포판(v0.5.0)에 실제로 포함된 것** (기본값은 꺼짐 — 아래 "켜는 방법" 먼저 확인하세요, **Windows·macOS만 해당**):
...
**아직 정식 배포 전, 최신 개발 상태에만 있는 것**(GitHub 저장소 소스 코드 기준 — 다음 릴리스에 포함될 예정이지만 지금 다운로드하시는 v0.5.0에는 없습니다):
- `claudetower accounts switch <라벨>` — 등록해둔 계정 중 하나로 **사람이 직접 명령을 실행해서** 전환합니다(자동 감지가 아니라 수동 전환). 확인 절차 없이 즉시 적용되며(언제든 다시 `switch`로 되돌릴 수 있어 삭제처럼 위험하지 않음), 로그인(구독) 계정으로는 전환할 수 없고 API 키 계정 사이에서만 가능합니다. 전환할 때마다 "언제·어느 계정에서 어느 계정으로 바뀌었는지" 기록이 함께 남습니다

**여전히 아예 없는 것**:
- 로그인(구독) 계정 자동 등록·자동 순환 — 위 이용약관 문제로 코드 자체가 없습니다
- 이미 로그인된 Claude Code 계정을 그대로 가져오는 기능(`--import`)
- 사용량 감지 후 **프로그램이 스스로 알아서 전환해주는 기능(완전 자동)** — 계정을 여러 개 등록해두어도, 지금은 사용량이 다 찼을 때 프로그램이 저절로 다음 계정으로 바꿔주는 일은 일어나지 않습니다. 사람이 직접 `accounts switch` 명령을 실행해야만 바뀝니다(위 참고, 이마저도 아직 정식 배포 전입니다)
...
정식 출시(릴리스)된 버전별 변경 내용입니다(현재 최신: v0.5.0). 항목을 클릭하면 펼쳐집니다.

<details>
<summary><strong>v0.5.0</strong> — 계정 등록·관리 기능 정식 포함 (최신)</summary>
...
- **계정 자동전환 기능을 만들 계획이 있나요?** 로그인(구독) 계정을 자동으로 순환시키는 기능은 Anthropic 이용약관과 충돌한다는 판단이 확인돼 있어 만들지 않습니다. 다만 이용약관이 예외로 인정하는 API 키 계정을 "등록"하는 기능은 v0.5.0부터 이미 실험적으로 포함되어 있으며 기본값은 꺼져 있습니다. 등록해둔 계정 사이를 사람이 직접 명령으로 전환하는 기능(`accounts switch`)은 다음 릴리스부터 포함될 예정이고, 사용량을 프로그램이 스스로 감지해서 완전 자동으로 전환해주는 기능은 여전히 계획에 없습니다(자세한 내용은 위 "② 계정 자동전환" 참고).

...
- `claudetower config padding <n>`(신규) — Claude Code 공식 상태표시줄 기능인 좌우 여백(문자 수)을 조절합니다(기본값 0). 예: `claudetower config padding 2`
...
정식 출시(릴리스)된 버전별 변경 내용입니다(현재 최신: v0.4.0). 항목을 클릭하면 펼쳐집니다. **전체 이력과 상세 설명은 [GUIDE.md 11장](./GUIDE.md#11-버전별-업데이트-요약)을 참고하세요.**
...
- **`/claudetower-widgets`(채팅으로 위젯 켜고 끄기)가 갑자기 안 돼요**: 근본 원인을 확정했습니다 — 이 프로그램 자신의 자동 검증 절차가 실수로 실제 설정 파일을 지우는 결함이었습니다(사용자 잘못이 아닙니다). 재발 방지 코드와, 혹시 사라져도 몇 초 안에 스스로 되살리는 자가복구를 모두 적용했습니다(v0.2.0부터 포함됨). 그보다 이전 버전을 쓰고 계시다면 최신 버전으로 업데이트하시고, 지금 버전에서 겪으신다면 `claudetower setup`을 다시 실행하면 즉시 복구됩니다.
...
- 라이선스(Apache 2.0)는 상업적 사용을 법적으로 금지하지 않습니다. 다만 이 프로젝트는 상업적 사용을 권장하지 않습니다 — 둘의 차이는 [`GUIDE.md`](./GUIDE.md) 17-7 참고.
- 이 프로젝트의 코드·문서 일부는 AI(Claude Code)의 도움을 받아 작성되었습니다(커밋 기록에 "Co-Authored-By: Claude" 명시). AI 보조 콘텐츠의 저작권·출처는 법리가 아직 확립되지 않은 영역이니, 필요 시 직접 확인하세요.
- 런타임 의존성이 0개라 재배포되는 제3자 코드가 없으므로 별도의 `NOTICE` 파일을 두지 않습니다.

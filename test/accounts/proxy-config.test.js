...
test('createProxyConfig: port 1024 미만(OS 예약 포트 영역)은 거부한다(2026-07-11 원본 스펙 재확인 회귀)', () => {
  assert.throws(() => createProxyConfig({ ...VALID, port: 500 }), TypeError);
  assert.throws(() => createProxyConfig({ ...VALID, port: 1023 }), TypeError);
  assert.doesNotThrow(() => createProxyConfig({ ...VALID, port: 1024 }));
});

test('createProxyConfig: thresholdPct 50 미만(너무 잦은 전환)은 거부한다(2026-07-11 원본 스펙 재확인 회귀)', () => {
  assert.throws(() => createProxyConfig({ ...VALID, thresholdPct: 30 }), TypeError);
  assert.throws(() => createProxyConfig({ ...VALID, thresholdPct: 49.9 }), TypeError);
  assert.doesNotThrow(() => createProxyConfig({ ...VALID, thresholdPct: 50 }));
});

...
test('ensureInstalledAtTarget: 개발 모드(node로 직접 실행)에서는 복사를 건너뛴다(SEA 아닐 때 "설치"라는 개념 자체가 없음)', () => {
  // node --test로 돌아가는 이 테스트 프로세스 자체가 SEA가 아니므로 isSea()===false —
  // 즉 이 테스트는 실제 SEA 바이너리 안에서만 유효한 "복사 발생" 경로는 검증할 수
  // 없다(단위 테스트의 한계 — SEA 복사 로직 자체는 빌드된 실제 바이너리로 별도 확인).
  const result = ensureInstalledAtTarget();

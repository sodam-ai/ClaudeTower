...
// 깨지는 걸 재배선 직후 직접 확인했다. git과 무관한 테스트는 전부 NON_GIT_WIDGET_TYPES를
// 쓰고, git 위젯 자체의 통합 동작은 이 파일 맨 아래 전용 테스트 2개로 별도 검증한다.
...
test('model 필드가 없으면 model 위젯만 숨겨지고 나머지는 그대로 표시된다', () => {
  const out = render({ workspace: { current_dir: '/x' } }, NON_GIT_WIDGET_TYPES);
  assert.doesNotMatch(out, /모델/);
  assert.match(out, /x/);
});

test('빈 세션(필드 전무)이면 빈 문자열을 반환한다', () => {
  assert.equal(render({}, NON_GIT_WIDGET_TYPES), '');
});
...
test('rate_limits.seven_day만 없으면 five_hour만 표시된다', () => {
  const out = render({ rate_limits: { five_hour: { used_percentage: 10 } } }, NON_GIT_WIDGET_TYPES);
...
test('context 70% 이상이면 경고색, 90% 이상이면 위험색, 그 아래는 안전색(초록)이 적용된다', () => {
  ...
  // (이전엔 안전 구간이 무색이었음).
  const warn = render({ context_window: { used_percentage: 75 } }, NON_GIT_WIDGET_TYPES);
  const critical = render({ context_window: { used_percentage: 95 } }, NON_GIT_WIDGET_TYPES);
  const safe = render({ context_window: { used_percentage: 10 } }, NON_GIT_WIDGET_TYPES);
...
test('위젯 하나가 예외를 던져도 나머지 위젯은 정상 렌더링된다(위젯 단위 격리)', () => {
  ...
  try {
    const out = render({ cost: { total_cost_usd: 2 } }, NON_GIT_WIDGET_TYPES);
...
test('경계값: context 정확히 70/90에서 각각 경고색/위험색, 69는 안전색(초록)/89는 경고색 유지', () => {
  ...
  assert.match(render({ context_window: { used_percentage: 69 } }, NON_GIT_WIDGET_TYPES), /\x1b\[32m/);
  assert.match(render({ context_window: { used_percentage: 70 } }, NON_GIT_WIDGET_TYPES), /\x1b\[33m/);
  assert.match(render({ context_window: { used_percentage: 89 } }, NON_GIT_WIDGET_TYPES), /\x1b\[33m/);
  assert.match(render({ context_window: { used_percentage: 90 } }, NON_GIT_WIDGET_TYPES), /\x1b\[31m/);
});
...
test('render(): enabledWidgets에서 git을 빼면 실제 저장소 안에서도 git 관련 출력이 전혀 없다', () => {
  const out = render({ model: { display_name: 'M' } }, NON_GIT_WIDGET_TYPES);
  assert.doesNotMatch(out, /🌿/);
});

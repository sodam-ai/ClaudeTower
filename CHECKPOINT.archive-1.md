# ClaudeTower — CHECKPOINT 아카이브 1 (M1~M29)

> 이 파일은 `CHECKPOINT.md`가 3,000줄(280KB)을 넘어 표준 파일 읽기 도구의 한도(256KB)를
> 초과하기 시작해(2026-08-20 이 프로젝트 세션 중 직접 겪은 실제 도구 오류), 오래되고
> 안정적으로 완료된 초기 마일스톤을 분리 보관하기 위해 만들었다. **내용은 한 글자도
> 삭제·수정하지 않았다 — `CHECKPOINT.md`에서 그대로 잘라 옮기기만 했다.**
>
> 범위: M1(2026-07-06)부터 M29(2026-08-04)까지 — Display 모듈(상태표시줄) 구축·설치
> 스크립트 결함 수정·1주 안정성 관찰 게이트·Account 모듈 안전지대 초기 준비 과정.
>
> **최신 마일스톤(M30~)과 최상단 안전 배너("🛑 실거래 배선 승인 게이트")는 이 파일이
> 아니라 저장소 루트의 `CHECKPOINT.md`에 있다 — 새 세션은 반드시 그쪽을 먼저 읽을 것.**
